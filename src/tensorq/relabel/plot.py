...
def plot_committor_confidence_cv(pack, q_values, results, config, out_dir):
    ...
    confidence = 1.0 - (-np.sum(np.clip(q_values, 1e-12, 1.0) * np.log(np.clip(q_values, 1e-12, 1.0)), axis=1) / np.log(q_values.shape[1]))
    entropy_norm = 1.0 - confidence

    for field_name, field, cmap_name, cbar_label in [
